...
const seedCourses = () => {
  ...
  if (courseCount.count === 0) {
    ...
    const pythonId = Number(insertCourse.run('Python Mastery', 'From fundamentals to advanced OOP and data structures.', 'https://picsum.photos/seed/python/800/600').lastInsertRowid);
    ...
    // Python Questions
    insertQuestion.run(pythonId, 'What is the output of print(type([]))?', JSON.stringify(['<class \'list\'>', '<class \'tuple\'>', '<class \'dict\'>', '<class \'set\'>']), '<class \'list\'>', 'basic');
    insertQuestion.run(pythonId, 'How do you define a function in Python?', JSON.stringify(['def', 'function', 'func', 'define']), 'def', 'basic');
    insertQuestion.run(pythonId, 'What is a decorator in Python?', JSON.stringify(['A function that modifies another function', 'A CSS class', 'A design pattern', 'A type of loop']), 'A function that modifies another function', 'advanced');
    insertQuestion.run(pythonId, 'What is the purpose of __init__ in a class?', JSON.stringify(['Constructor method', 'Destructor method', 'Static method', 'Private method']), 'Constructor method', 'advanced');
    insertQuestion.run(pythonId, 'Which of these is used for multi-threading in Python?', JSON.stringify(['threading module', 'os module', 'sys module', 'math module']), 'threading module', 'advanced');
    ...
    db.prepare('UPDATE courses SET questionCount = (SELECT COUNT(*) FROM questions WHERE courseId = ?) WHERE id = ?').run(pythonId, pythonId);
